...
export const SidebarUpdateItem: FC<SidebarUpdateItemProps> = ({
  ...
  const fetchWorkpaceFunctions = useCallback(
    () => ({
      chats: null,
  ...
  const fetchSelectedWorkspaces = useCallback(async () => {
    ...
    return workspaces
  }, [contentType, item.id, fetchWorkpaceFunctions])

  const fetchDataFunctions = useCallback(
    () => ({
      chats: null,
      presets: null,
      prompts: null,
  ...
  const updateFunctions = {
    ...
    "news-chats": async (chatId: string, updateState: any) => {
      const updatedChat = await updateNewsChat(chatId, updateState)
      return updatedChat
    }
  }
  ...
  const stateUpdateFunctions = {
    ...
    tools: setTools,
    models: setModels,
    "news-chats": setNewsChats
  }

  const handleUpdate = async () => {
    try {
      const updateFunction = (updateFunctions as any)[contentType]
      const setStateFunction = (stateUpdateFunctions as any)[contentType]
